import antropioLogo from '@/assets/images/companies/antropio.svg'
import auctaneLogo from '@/assets/images/companies/auctane.svg'
import codestorybroLogo from '@/assets/images/companies/codestorybro.svg'
import trustmateLogo from '@/assets/images/companies/trustmate.svg'

export const logos = [
  {
    name: codestorybroLogo,
    alt: 'CodeStoryBro',
    link: 'https://codestorybro.com/',
  },
  {
    name: antropioLogo,
    alt: 'Antropio',
    link: 'https://www.linkedin.com/company/antropio',
  },
  { name: auctaneLogo, alt: 'Auctane', link: 'https://auctane.com/' },
  { name: trustmateLogo, alt: 'TrustMate', link: 'https://trustmate.io/' },
]
